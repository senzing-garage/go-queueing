...
1. Identify git repository.
    ...
    export GIT_REPOSITORY=go-queueing
...
1. Run program.
   Examples:
    ...
    1. Linux
        ...
        ```console
        ${GIT_REPOSITORY_DIR}/target/linux-amd64/go-queueing
    ...
    1. macOS
        ...
        ```console
        ${GIT_REPOSITORY_DIR}/target/darwin-amd64/go-queueing
    ...
    1. Windows
        ...
        ```console
        ${GIT_REPOSITORY_DIR}/target/windows-amd64/go-queueing
...
1. Senzing documentation will be in the "Third party" section.
   `github.com` > `senzing-garage` > `go-queueing`
...
1. Run docker container.
   Example:
    ...
    ```console
    docker run --rm senzing/go-queueing
...
1. Determine if `go-queueing` is installed.
   Example:

    ```console
    apt list --installed | grep go-queueing

    ```

1. :pencil2: Install `go-queueing`.
   Example:

    ```console
    cd ${GIT_REPOSITORY_DIR}/target
    sudo apt install ./go-queueing-0.0.0.deb
...
1. Remove `go-queueing` from system.
   Example:

    ```console
    sudo apt-get remove go-queueing
